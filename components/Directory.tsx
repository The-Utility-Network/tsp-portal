...
import { getFacets, diamondAddress as getDiamondAddress } from '../primitives/Diamond';
  ...
  return (
    <div
      ...
      style={{
        left: position.left,
        top: position.top,
        backgroundColor: isRead ? 'rgba(0, 20, 26, 0.85)' : 'rgba(26, 0, 13, 0.85)',
        borderColor: isRead ? 'rgba(0, 204, 255, 0.3)' : 'rgba(255, 105, 180, 0.3)', // Cyan/Pink border
        boxShadow: executing ? `0 0 30px ${isRead ? '#00ccff' : '#ff69b4'}` : '0 10px 30px rgba(0,0,0,0.5)',
      ...
      <div
        ...
        style={{
          backgroundImage: isRead
            ? 'linear-gradient(90deg, rgba(0, 204, 255, 0.15), transparent)'
            : 'linear-gradient(90deg, rgba(255, 105, 180, 0.15), transparent)'
        }}
      >
        <div
        ...
        >
          {isRead ? <PlayCircleIcon className="w-5 h-5 text-brand-500" /> : <BoltIcon className="w-5 h-5 text-accent" />}
      ...
      <div className="p-4 space-y-4 relative">
            ...
            {inputs.map((input, idx) => (
              <div key={idx} className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  style={{
                    borderColor: isRead ? 'rgba(0, 204, 255, 0.2)' : 'rgba(255, 105, 180, 0.2)'
                  }}
                  onFocus={(e) => e.target.style.borderColor = isRead ? '#00ccff' : '#ff69b4'}
                  onBlur={(e) => e.target.style.borderColor = isRead ? 'rgba(0, 204, 255, 0.2)' : 'rgba(255, 105, 180, 0.2)'}
                  placeholder="Value..."
                  value={input.value}
        ...
        <div className="pt-2 flex gap-2">
          <button
            ...
            style={{
              background: isRead ? 'rgba(0, 204, 255, 0.15)' : 'rgba(255, 105, 180, 0.15)',
              border: `1px solid ${isRead ? 'rgba(0, 204, 255, 0.4)' : 'rgba(255, 105, 180, 0.4)'}`,
              color: isRead ? '#00ccff' : '#ff69b4',
            }}
          >
      ...
      {outputModalOpen && (
        <div className="absolute inset-0 bg-black/95 z-[60] flex flex-col p-4 animate-in fade-in zoom-in duration-200">
          <div className="flex justify-between items-center mb-2 pb-2 border-b border-white/10">
            <span className="text-xs font-mono text-brand-400 uppercase tracking-widest">Execution Result</span>
            <button onClick={() => setOutputModalOpen(false)} className="text-white hover:text-red-500">×</button>
          </div>
          <div className="flex-1 overflow-auto custom-scrollbar">
            <pre className="text-[10px] font-mono text-brand-300 whitespace-pre-wrap leading-relaxed">
              {outputContent}
            </pre>
          </div>
        </div>
    ...
    >
      ...
      <div className="absolute top-4 right-4 z-[100] flex gap-2">
        <div className="flex bg-black/50 backdrop-blur-md border border-white/10 rounded-lg p-1">
          <button onClick={() => setScale(s => Math.min(s * 1.2, 3))} className="p-2 hover:bg-white/10 rounded text-white/70 hover:text-brand-500"><MagnifyingGlassPlusIcon className="w-5 h-5" /></button>
          <button onClick={() => setScale(s => Math.max(s / 1.2, 0.2))} className="p-2 hover:bg-white/10 rounded text-white/70 hover:text-brand-500"><MagnifyingGlassMinusIcon className="w-5 h-5" /></button>
          <div className="w-[1px] bg-white/10 mx-1" />
          <button onClick={() => { setScale(1); setPosition({ x: 0, y: 0 }); }} className="p-2 hover:bg-white/10 rounded text-white/70 hover:text-brand-500"><ArrowPathIcon className="w-5 h-5" /></button>
      ...
      {panels.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-20">
          <div className="flex flex-col items-center">
            <CubeTransparentIcon className="w-24 h-24 text-brand-500 mb-4" />
      ...
      {clearModalOpen && (
        <div className="absolute inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[#050a14] border border-brand-500/30 rounded-lg p-6 max-w-sm w-full shadow-[0_0_30px_rgba(0,204,255,0.2)]">
            <h3 className="text-brand-500 font-mono font-bold text-lg mb-2 tracking-widest">CLEAR WORKSPACE</h3>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                className="px-4 py-2 rounded bg-brand-500/10 border border-brand-500/50 text-brand-500 hover:bg-brand-500/20 text-[10px] font-bold tracking-wider transition-all shadow-[0_0_20px_rgba(0,204,255,0.1)] hover:shadow-[0_0_30px_rgba(0,204,255,0.3)]"
              >
                CONFIRM_CLEAR
              </button>
            </div>
...
const MethodItem = ({ methodName, isRead, inputs, outputs }: any) => {
  ...
  return (
    <div ref={drag as unknown as React.LegacyRef<HTMLDivElement>} className={`group p-3 rounded-md border cursor-grab transition-all duration-200 relative overflow-hidden ${isDragging ? 'opacity-50' : 'opacity-100 hover:scale-[1.02]'
        ...
        ? 'bg-brand-500/10 border-brand-500/20 hover:border-brand-500/60'
        : 'bg-accent/10 border-accent/20 hover:border-accent/60'
      }`}>
      <div className={`absolute left-0 top-0 bottom-0 w-1 ${isRead ? 'bg-brand-500' : 'bg-accent'} opacity-50`} />
      <div className="flex justify-between items-center pl-2">
        <span className={`font-mono text-[11px] font-bold truncate ${isRead ? 'text-brand-500' : 'text-accent/80'}`}>
          {methodName}
        </span>
    ...
    <DndProvider backend={HTML5Backend}>
      <div
        ...
        style={{
          ...
          border: '1px solid rgba(0, 204, 255, 0.2)', // Cyan Border
          boxShadow: '0 0 40px rgba(0, 204, 255, 0.1)', // Cyan Glow
        }}
      >
        {/* Workspace - Takes FULL height on mobile minus the bottom bar, full width on right on desktop */}
        <div className="flex-1 relative bg-[#00141a] order-1 md:order-2">
          <Workspace allABIs={allABIs} diamondAddress={diamondAddress} />
        </div>

        {/* Sidebar / Bottom Bar */}
        <div className="w-full md:w-72 h-32 md:h-full flex-shrink-0 flex flex-col border-t md:border-t-0 md:border-r border-brand-500/20 bg-[#00141a]/90 backdrop-blur-xl z-20 order-2 md:order-1">
          <div className="p-3 md:p-5 border-b border-brand-500/20 bg-black/20 flex justify-between md:block items-center">
            <div>
              ...
              <div className="text-[9px] md:text-[10px] text-brand-500 font-mono tracking-widest flex items-center gap-2">
          ...
          <div className="flex-1 overflow-x-auto md:overflow-y-auto overflow-y-hidden md:overflow-x-hidden p-2 md:p-3 space-x-4 md:space-x-0 md:space-y-6 custom-scrollbar flex flex-row md:flex-col items-center md:items-stretch">
            {loading ? (
              <div className="text-center py-4 md:py-20 space-x-4 md:space-x-0 md:space-y-4 flex md:block items-center justify-center w-full">
                <div className="w-6 h-6 md:w-10 md:h-10 border-2 border-brand-500 border-t-transparent rounded-full animate-spin mx-auto" />
                <div className="text-[10px] font-mono text-brand-500 animate-pulse tracking-widest whitespace-nowrap">
